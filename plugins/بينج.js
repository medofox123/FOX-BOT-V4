...
let enlace2 = { contextInfo: { externalAdReply: { showAdAttribution: true, mediaUrl: yt, mediaType: 'VIDEO', description: '', title: wm, body: '𝗙𝗢𝗫 𝗪𝗛𝗔𝗧"𝗦𝗔𝗣𝗣 𝗕𝗢𝗧 ', thumbnailUrl: await(await fetch(gataMenu)).buffer(), sourceUrl: accountsgb }}}
...
await conn.reply(m.chat, `*🚀 انــــتـظࢪ جـــاࢪ الـــحـساب*...`, fkontak, {contextInfo: { externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: gt, body: '𝗙𝗢𝗫 𝗪𝗛𝗔𝗧"𝗦𝗔𝗣𝗣 𝗕𝗢𝗧 ', previewType: 0, thumbnail: gataImg, sourceUrl: accountsgb }}})
//await conn.sendButton(m.chat, wm, `${eg}🚀 𝙋𝙍𝙐𝙀𝘽𝘼 𝘿𝙀 𝙑𝙀𝙇𝙊𝘾𝙄𝘿𝘼𝘿...\n🚀 𝙎𝙋𝙀𝙀𝘿 𝙏𝙀𝙎𝙏...`, null, [['𝗠 𝗘 𝗡 𝗨 ☘️', '/menu']], fkontak, m)  

let neww = performance.now()
let speed = neww - old
let caption = `*ســـــࢪعــــه الــــبوت*

🚄 *${Math.round(neww - old)}* ms
🚄 *${speed}* ms

*${htjava}* زمـن التــشــغــــيل 
${muptime}
${readMore}
...
handler.command = /^(بينج|speed|velocidad|rapidez|velocity)$/i
...
function clockString(ms) {
  ...
  return [' ' + d, ' *يــــــوم ☀️*\n ', h, ' *ســـــــاعـه 🕐*\n ', m, ' *دقــــيقـــه ⏰*\n ', s, ' *ثـــانـــــيه ⏱️* '].map(v => v.toString().padStart(2, 0)).join('')
}
