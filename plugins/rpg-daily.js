...
if (new Date - user.lastclaim < 7200000) return await conn.reply(m.chat, `لقد طلبت بالفعل هديتك 🎁\n عد في *${msToTime(time - new Date())}* للمطالبة مرة أخرى`, fkontak,  m)
//await conn.sendButton(m.chat, `لقد طلبت بالفعل هديتك 🎁\nعد في *${msToTime(time - new Date())}*للمطالبة مرة أخرى`, wm, null, [['𝗠 𝗘 𝗡 𝗨 ☘️', '/menu']], fkontak, m)
let texto = ''
...
//await conn.sendButton(m.chat, text, texto + `\n\n🎟️ بريميوم ⇢ ${premium ? '✅' : '❌'}\n${wm}`, img, [['⛰️ 𝘼𝙗𝙧𝙞𝙧 𝙘𝙤𝙛𝙧𝙚  ⛰️', '/cofre'], ['الاوامر ☘️', '/menuall'] ], fkontak, m)  
user.lastclaim = new Date * 1
}
handler.help = ['daily']
handler.tags = ['xp']
handler.command = ['daily', 'reclamar', 'يومي', 'مجاني', 'جمع'] 
